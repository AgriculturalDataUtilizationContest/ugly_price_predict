...
# KAMIS 가격 정보 조회
app.include_router(kamis_router,     prefix="/pyapi", tags=["kamis"])
# 과거 비율 기반 조정비용 계산
app.include_router(past_ugly_router, prefix="/pyapi", tags=["ugly"])
# 학습된 모델을 이용한 예측
app.include_router(predict_router,   prefix="/pyapi", tags=["predict"])
app.include_router(future_calc_router, prefix="/pyapi", tags=["future_calc"])
app.include_router(pred_all_router, prefix="/pyapi", tags=["pred_all"])
# wordcloud 생성 및 Spring Boot로 전송
app.include_router(generate_router, prefix="/pyapi", tags=["generate"])
